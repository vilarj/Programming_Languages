...
public class parser {
    ...
    public static void read_file(FileReader file) {
        ...
        if (file == null){System.out.println("ERROR - cannot open codes.txt");}
        else{
            try {
                while(reader.ready()){
                    getChar();
            ...
            if (nextToken == LEFT_PAREN) {nextToken = '('; System.out.printf("\t\t%c", token_dict[nextToken]);}
            if (nextToken == RIGHT_PAREN) {nextToken = ')'; System.out.printf("\t\t%c", token_dict[nextToken]);}
            if (nextToken == ADD_OP) {nextToken = '+'; System.out.printf("\t\t%c", token_dict[nextToken]);}
            if (nextToken == SUB_OP) {nextToken = '-'; System.out.printf("\t\t%c", token_dict[nextToken]);}
